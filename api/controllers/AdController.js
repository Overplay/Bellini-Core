...
var AD_RUNNING = { reviewState: 'Accepted', deleted: false, paused: false };
...
module.exports = {
    ...
    forVenue: function ( req, res ) {
        ...
        const uuid = params.id || params.venueUUID;

        if ( !uuid )
            return res.badRequest( { error: "No venue uuid provided" } );

        return Venue.findOne( { uuid: uuid } )
            .then( function ( venue ) {
                if ( !venue )
                    return res.notFound( { error: "Venue with uuid " + uuid + " not found" } );

                if ( !venue.sponsorships )
                    return res.ok( [] );

                // `sponsorships` is an array of Mongo IDs
                const query = _.extend( AD_RUNNING, { id: venue.sponsorships } );
